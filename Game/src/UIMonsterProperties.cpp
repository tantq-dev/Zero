...
void Tool::UI::UIMonsterProperties::BehaviorShootBarrageConfigUI(BehaviorNode& node)
{
    if (auto* config = dynamic_cast<BehaviorShootBarrageConfig*>(node.config.get())) {
        ...
        int bulletType = static_cast<int>(config->bulletType);
        if (ImGui::Combo("Bullet Type", &bulletType, bulletTypeNames, IM_ARRAYSIZE(bulletTypeNames))) {
            config->bulletType = static_cast<BulletType>(bulletType);
        }
...
std::unique_ptr<Tool::UI::BehaviorNode> Tool::UI::UIMonsterProperties::ConvertBehaviorConfigToNode(const BehaviorConfig* config)
{
    ...
    if (auto* chase = dynamic_cast<const BehaviorChaseConfig*>(config)) {
        ...
        configCopy->bulletType = barrage->bulletType;
        ...
        configCopy->bulletType = projectile->bulletType;
        node->config = std::move(configCopy);
    }
        ...
        configCopy->bulletType = spread->bulletType;
...
void Tool::UI::UIMonsterProperties::ConvertToMonsterProperties(MonsterProperties& properties)
{
    ...
    // Log conversion for debugging
    std::cout << "[" << "2025-06-11 17:45:06" << "] ConvertToMonsterProperties: Converted "
        << properties.rootBehavior->childBehaviors.size() << " behaviors to monster: "
        << properties.name << std::endl;
}
